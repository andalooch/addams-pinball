...
function buildAudio(){
  ...
  const sfx={
    ...
    bonus(){[523,659,784,1047,1319].forEach((f,i)=>osc(sG,'square',f,0.2,0.22,f*0.9,undefined,ac.currentTime+i*0.07));},
    ...
    gameover(){[350,260,180,120].forEach((f,i)=>osc(sG,'sawtooth',f,0.28,0.4,f*0.7,undefined,ac.currentTime+i*0.18));},
    thing(){[300,400,500,700,1000].forEach((f,i)=>osc(sG,'square',f,0.25,0.18,f*1.2,undefined,ac.currentTime+i*0.04));nz(sG,0.3,0.2,600,2);},
    ...
    tilt(){[200,150,100].forEach((f,i)=>osc(sG,'sawtooth',f,0.4,0.3,f*0.6,undefined,ac.currentTime+i*0.12));},
    multiball(){[262,330,392,523].forEach((f,i)=>osc(sG,'square',f,0.25,0.3,f,undefined,ac.currentTime+i*0.08));},
    kbRecharge(){osc(sG,'sine',660,0.2,0.2,880);osc(sG,'sine',880,0.15,0.15,1047,undefined,ac.currentTime+0.1);},
    // New SFX
    ramp(side:'left'|'right'){const b=side==='left'?1:1.2;[400,600,900].forEach((f,i)=>osc(sG,'square',f*b,0.22,0.2,f*b*1.1,undefined,ac.currentTime+i*0.06));},
    drop(){osc(sG,'square',220,0.3,0.08,110);nz(sG,0.2,0.05,400,1);},
    dropComplete(){[300,450,600,900].forEach((f,i)=>osc(sG,'square',f,0.2,0.15,f*1.1,undefined,ac.currentTime+i*0.055));},
    spin(){nz(sG,0.1,0.03,1500,0.5);osc(sG,'sine',800,0.08,0.04,400);},
    topLane(){osc(sG,'sine',660,0.2,0.15,880);},
    topLaneAll(){[523,659,784,1047].forEach((f,i)=>osc(sG,'sine',f,0.2,0.25,f,undefined,ac.currentTime+i*0.06));},
    orbit(){[800,1000,1200].forEach((f,i)=>osc(sG,'sine',f,0.2,0.2,f*1.1,undefined,ac.currentTime+i*0.05));},
    magnetCapture(){osc(sG,'sine',60,0.5,0.8,40);nz(sG,0.2,0.3,200,0.5);[300,200,100].forEach((f,i)=>osc(sG,'sine',f,0.15,0.4,f*0.6,undefined,ac.currentTime+i*0.08));},
    magnetRelease(){[200,400,600,1000,1400].forEach((f,i)=>osc(sG,'square',f,0.22,0.25,f*1.1,undefined,ac.currentTime+i*0.055));osc(sG,'sine',1200,0.3,0.5,2000,undefined,ac.currentTime+0.3);},
